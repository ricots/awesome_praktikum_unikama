...
<input class="submit" type="submit" value="TAMBAH OBAT" />
</form>
<br />
<?php  
include "../../connect.php";
...
if(empty($halaman)){
   ...
   $sql = "SELECT * FROM obat WHERE $skripsi LIKE '%$search%' ORDER BY kd_obat ASC LIMIT $posisi,$batas ";
   $no=$posisi + 1;
...
<table border="1">
<tr>
<th>No</th><th>Kd Obat</th><th>Nama Obat</th><th>Jumlah Obat</th><th>Ukuran</th><th>Harga</th><th>Aksi</th></tr>
	<?php
	...
	echo "<td>
	<a href=form_edit.php?kd_obat=$row[kd_obat]>Edit</a>&nbsp; | 
	<a href=\"hapus_obat.php?kd_obat=$row[kd_obat]\"onClick=\"return confirm('Apakah Anda benar-benar akan menghapus $row[nm_obat]?')\">Hapus</a>
	</td></tr>";
	 $no++;
...
echo "<br>Halaman :&nbsp;";
	  ...
	  { $skripsi = $_GET['kombo'];}
$tampil2="SELECT * FROM obat WHERE $skripsi LIKE '%$search%' ORDER BY kd_obat ASC";
