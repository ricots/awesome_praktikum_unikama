...
if($op=='ambilbarang'){
    ...
    echo $d['nama']."|".$d['totalberat']."|".$d['jumlah'];
}elseif($op=='barang'){
    ...
    $tot=$to['total'];
    $simpan=mysql_query("insert into pembayaran(nonota,tanggal,total)
                        values ('$nota','$tanggal','$tot')");
    if($simpan){
        $query=mysql_query("select * from tblsementara");
        while($r=mysql_fetch_row($query)){
            mysql_query("insert into detail_pembayaran(nonota,kode,harga,jumlah,subtotal)
